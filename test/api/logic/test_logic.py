...
class TestLogic(unittest.TestCase):
    ...
    def test_handle_login_valid_credentials(self, mock_create_access_token, mock_verify_password, mock_find_one_user):
        # Arrange
        ...
        # Act
        token = Logic.handle_login(login_data)
    ...
    @patch('api.auth.auth_service.AuthService.create_access_token')
    def test_handle_signup_new_user(self, mock_create_access_token, mock_hash_password, mock_create_user):
        # Arrange
        ...
        # Act
        token = Logic.handle_signup(signup_data)
    ...
    # Test Logic for Handle Token
    @patch('api.auth.auth_service.AuthService.verify_token')
    def test_handle_token_valid(self, mock_verify_token):
        # Arrange
        token = "valid_token"
        mock_verify_token.return_value = {"sub": "test@example.com", "userType": "admin"}
    ...
    @patch('api.auth.auth_service.AuthService.verify_token')
    def test_handle_token_invalid(self, mock_verify_token):
        # Arrange
        token = "invalid_token"
        mock_verify_token.side_effect = JWTError("Token invalid")
    ...
    def test_get_current_user_valid(self, mock_find_one_user, mock_verify_token):
        ...
        mock_find_one_user.assert_called_once_with({"email": "test@example.com", "userType": "admin"})
