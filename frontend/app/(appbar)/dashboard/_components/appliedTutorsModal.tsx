    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white w-full max-w-xl md:mx-0 mx-4 rounded-lg shadow-lg relative">
        ...
        <div className="flex justify-between items-center px-4 md:px-6 py-4 border-b">
          ...
          <button
            onClick={onClose}
            className="text-customOrange hover:text-red-700"
          >
            <X size={20} />
          </button>
        </div>
          ...
          {requests?.map((request) => (
            ...
            >
              ...
              <div className="flex flex-col md:flex-row gap-2 md:ml-auto">
                <Button
                  className="px-4 py-2 flex justify-center bg-customYellow text-white rounded-full hover:bg-customOrange transition-colors duration-200 text-sm"
                  onClick={() => onAccept(request.id)}
                >
                  Accept
                </Button>
                <Button
                  className="px-4 py-2 flex justify-center bg-white border border-orange-400 text-orange-400 text-sm rounded-full"
