...
export function Pagination({ totalPages }: PaginationProps) {
  ...
  return (
    <div className="flex items-center justify-center space-x-2 text-md">
      ...
      {currentPage > 1 && (
        <Link href={createPageURL(currentPage - 1)} className="bg-white px-2 py-1 border rounded-lg hover:bg-gray-100">
          « Prev
        </Link>
      )}
      {/* Page Number Links */}
      {pageNumbers.map((page) => (
        <Link
          ...
          className={`px-3 py-1 border rounded-lg ${page === currentPage ? 'bg-customDarkBlue text-white border-customDarkBlue' : 'hover:bg-gray-100 bg-white'}`}
        >
          {page}
        </Link>
      ))}
      ...
      {currentPage < totalPages && (
        <Link href={createPageURL(currentPage + 1)} className="bg-white px-2 py-1 border rounded hover:bg-gray-100">
          Next » 
        </Link>
      )}
    </div>
