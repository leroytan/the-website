...
export default function TutorBrowsePage() {
  ...
  const allLevels = Array.from(
    new Set(mockTutors.flatMap((tutor) => tutor.levels))
  );
  ...
  const filteredTutors = mockTutors.filter(
    (tutor) =>
      ...
      (selectedLevels.length === 0 ||
        tutor.levels.some((level) => selectedLevels.includes(level)))
  );
    ...
    <div className={`min-h-screen bg-[#fff2de] ${inter.className}`}>
      <main className="pt-16 sm:pt-20 md:pt-24 pb-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-4xl font-bold text-[#4a58b5] mb-8 text-center">
            THE Tutors
          </h1>
          ...
          <AnimatePresence>
            {showFilters && (
              ...
              >
                <h3 className="text-lg font-semibold text-[#4a58b5] mb-4">
                  Filter by Subject:
                </h3>
