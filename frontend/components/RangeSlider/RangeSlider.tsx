...
const range = function (start: number | string, end: number | string, step: number): Array<number | string> {
    ...
    if (typeof start === "number" && typeof end === "number") {
        ...
        while (step > 0 ? endNum >= startNum : endNum <= startNum) {
            range.push(startNum);
            startNum = (startNum + step) as number;
        }
        ...
        let startCode = startStr.charCodeAt(0);
...
const RangeSlider: React.FC<RangeSliderProps> = ({ hasSteps, tooltipVisibility, tooltipPosition, value, onChange, from, to, formatter }) => {
    ...
    const updateValue = React.useCallback((closer: HTMLDivElement) => {
        ...
        setUpdate(prev => ({ ...prev, value: null }));
    }, [ballSize, railRef, trackRef, values, minRef, maxRef, setMin, setMax, setUpdate]);
    ...
    }, [values, outputRef, minTooltipRef, maxTooltipRef, trackRef, railRef]);
    ...
    }, [onChange, outputRef, tooltipVisibility]);
    ...
    useEffect(() => {
        ...
        outputRef.current = {
            min: min.value,
            max: max.value,
            minIndex: min.valueIndex,
            maxIndex: max.valueIndex
        };
    ...
    }, [merged, moving, lastMoved, tooltipVisibility]);
