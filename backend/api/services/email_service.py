...
from api.storage.models import Assignment
...
class GmailEmailService:
    ...
    def create_reset_link(
        reset_token: str, url: str = "https://yourwebsite.com/reset-password"
    ) -> str:
        """
    ...
    def create_email_confirmation_link(
        confirmation_token: str, url: str = "https://yourwebsite.com/confirm-email"
    ) -> str:
        """
    ...
    def send_email_confirmation_email(
        ...
        try:
            ...
            html_content = template.render(
                user_name=user_name, confirmation_link=confirmation_link
            )
            html_part = MIMEText(html_content, "html")
            message.attach(html_part)
            ...
            if attachments:
                for attachment in attachments:
                    ...
                    part = MIMEApplication(
                        file_content, _subtype=mimetype or "octet-stream"
                    )
                    part.add_header(
    ...
    def notify_new_assignment_request(
        ...
        try:
            ...
            html_content = template.render(assignment=assignment, origin=origin)
            html_part = MIMEText(html_content, "html")
            message.attach(html_part)
