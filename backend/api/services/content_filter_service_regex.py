...
class ContentFilterService:
    ...
    def _load_singapore_roads(self) -> List[str]:
        ...
        try:
            ...
            logging.warning(
                f"Road names CSV file not found at {csv_path}, using fallback list"
            )
            ...
            logging.error(
                f"Error loading road names from CSV: {e}, using fallback list"
            )
            roads = [
                "ORCHARD ROAD",
                "MARINA BAY",
    ...
    def _initialize_patterns(self):
        ...
        self.patterns = {
            ...
            "sg_phone_strict": re.compile(
                r"\b[689]\d{7}\b"
            ),  # 8 digits starting with 6, 8, or 9
            "sg_phone_intl": re.compile(
                r"\+65[-.\s]?[689]\d{7}\b"
            ...
            # HDB/Block addresses
            "hdb_block": re.compile(
                rf"\b(?:Block|Blk)\s+\d+[A-Za-z]?\s+(?:{area_pattern}|[A-Za-z\s]+(?:Estate|Park|Gardens?|Heights?|View|Place|Court|Square|North|South|East|West|Central))\b",
                re.IGNORECASE,
            ),
            # Street addresses with road names
            "street_address": re.compile(
                rf"\b\d+[A-Za-z]?\s+(?:{road_pattern})\b", re.IGNORECASE
            ),
            # Unit numbers
            "unit_number": re.compile(
                r"#\d{2}-\d{2,3}|(?:unit|Unit)\s*[#]?\d{1,2}[-\s]?\d{1,3}\b|\b\d{1,3}-\d{1,3}\b",
                re.IGNORECASE,
            ),
        ...
        self.price_exclusions = re.compile(
            r"(?:\$|SGD|dollars?|cents?|price|cost|pay|paid|fee|charge|bill|rent|salary|wage|income|budget|total|amount)\s*\d+|\d+\s*(?:\$|SGD|dollars?|cents?)",
            re.IGNORECASE,
        )
    ...
    def _extract_context_around_match(
        self, message: str, match_start: int, match_end: int, window: int = 30
    ) -> str:
            ...
            for match in pattern.finditer(message):
                ...
                if has_price_context:
                    ...
                    if number_str.isdigit():
                        ...
                        if number <= 200 and any(
                            word in context
                            for word in ["$", "sgd", "dollar", "price", "cost", "pay"]
                        ):
                            continue

                # Skip if deny context without phone indicators
                if self._has_deny_context(context) and not any(
                    word in context
                    for word in ["call", "phone", "contact", "whatsapp", "text"]
                ):
                    continue
    ...
    def _filter_addresses(self, message: str) -> List[Dict]:
        ...
        for match in self.patterns["postal_code"].finditer(message):
            ...
            is_direct_price = any(
                word in context
                for word in ["$", "sgd", "dollar", "price", "cost", "pay", "paid"]
            )
            ...
            has_deny_context = (
                len(set(re.findall(r"\b\w+\b", context.lower())) & postal_deny_context)
                > 0
            )
        ...
        for match in self.patterns["unit_number"].finditer(message):
            ...
            is_price_context = any(
                word in context
                for word in ["price", "cost", "buy", "sell", "dollar", "sgd"]
            )
            ...
            has_sports_context = any(
                word in context for word in ["score", "match", "game", "vs", "against"]
            )
            has_academic_context = any(
                word in context for word in ["chapter", "section", "page", "lesson"]
            )

            # Allow standalone unit patterns or those with address context, but not price/sports/academic context
            if (
                (has_address_context or is_unit_pattern)
                and not is_price_context
                and not has_sports_context
                and not has_academic_context
            ):
                detected.append(
                    {
                        "type": "UNIT_NUMBER",
    ...
    async def filter_message(self, message: str, threshold: float = 0.7) -> Dict:
        ...
        for detection in sorted(
            all_detections, key=lambda x: (x["start"], -x["confidence"])
        ):
        ...
        high_confidence_detections = [
            d for d in unique_detections if d["confidence"] >= threshold
        ]

        if high_confidence_detections:
            # Create anonymized message
            anonymized_message = self._anonymize_message(
                message, high_confidence_detections
            )
            detected_types = list(set(d["type"] for d in high_confidence_detections))
            avg_confidence = sum(
                d["confidence"] for d in high_confidence_detections
            ) / len(high_confidence_detections)
