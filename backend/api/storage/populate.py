...
def generate_bulk_assignments(
    ...
    for i in range(count):
        ...
        assigned_tutor = (
            random.choice(tutors) if status == AssignmentStatus.FILLED else None
        )
        ...
        for day, start, end in random.sample(time_slots, k=random.choice([1, 2])):
            slots.append(
                AssignmentSlot(
                    assignment_id=assignment.id, day=day, start_time=start, end_time=end
                )
            )

        # Link at least one subject
        assignment_subjects.append(
            AssignmentSubject(assignment_id=assignment.id, subjectId=subject.id)
        )
...
def insert_test_data(engine: object) -> bool:
    ...
    tutor_populate_special_skills = [
        # Alice: Test Prep, AP
        TutorSpecialSkill(
            tutor_id=tutors[0].id, special_skill_id=populate_special_skills[0].id
        ),  # Test Prep
        TutorSpecialSkill(
            tutor_id=tutors[0].id, special_skill_id=populate_special_skills[2].id
        ),  # AP
        # Bob: Test Prep, AP, Project-Based Learning
        TutorSpecialSkill(
            tutor_id=tutors[1].id, special_skill_id=populate_special_skills[0].id
        ),  # Test Prep
        TutorSpecialSkill(
            tutor_id=tutors[1].id, special_skill_id=populate_special_skills[2].id
        ),  # AP
        TutorSpecialSkill(
            tutor_id=tutors[1].id, special_skill_id=populate_special_skills[3].id
        ),  # Project-Based
        # Carol: Homework Help, Career Counseling
        TutorSpecialSkill(
            tutor_id=tutors[2].id, special_skill_id=populate_special_skills[4].id
        ),  # Homework Help
        TutorSpecialSkill(
            tutor_id=tutors[2].id, special_skill_id=populate_special_skills[5].id
        ),  # Career Counseling
        # David: Project-Based Learning, Homework Help
        TutorSpecialSkill(
            tutor_id=tutors[3].id, special_skill_id=populate_special_skills[3].id
        ),  # Project-Based
        TutorSpecialSkill(
            tutor_id=tutors[3].id, special_skill_id=populate_special_skills[4].id
        ),  # Homework Help
        # Eva: AP, Special Education
        TutorSpecialSkill(
            tutor_id=tutors[4].id, special_skill_id=populate_special_skills[2].id
        ),  # AP
        TutorSpecialSkill(
            tutor_id=tutors[4].id, special_skill_id=populate_special_skills[1].id
        ),  # Special Education
        # Frank: ESL, Special Education
        TutorSpecialSkill(
            tutor_id=tutors[5].id, special_skill_id=populate_special_skills[6].id
        ),  # ESL
        TutorSpecialSkill(
            tutor_id=tutors[5].id, special_skill_id=populate_special_skills[1].id
        ),  # Special Education
    ]
    ...
    generate_bulk_assignments(
        session, users, tutors, subjects, levels, locations, count=50, seed=42
    )
