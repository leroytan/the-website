...
class AssignmentLogic:
    @staticmethod
    def convert_assignment_to_view(
        session: Session,
        assignment: Assignment,
        view_type: ViewType = ViewType.PUBLIC,
        user_id: int = None,
    ) -> AssignmentOwnerView | AssignmentPublicView:
        ...
        base_data = {
            ...
            "subjects": [subject.name for subject in assignment.subjects]
    ...
    def search_assignments(search_query: SearchQuery, user_id: int = None) -> dict:
        with Session(StorageService.engine) as session:
            ...
            if "subject" in parsed_filters:
                filters.append(
                    Assignment.subjects.any(
                        Subject.filter_id.in_(parsed_filters["subject"])
                    )
                )
            ...
            try:
                ...
                statement = statement.order_by(
                    SortLogic.get_sorting(Assignment, search_query.sort_by)
                )
            ...
            results = [
                AssignmentLogic.convert_assignment_to_view(
                    session, assignment, ViewType.PUBLIC, user_id
                )
    ...
    @staticmethod
    def new_assignment(
        new_assignment: NewAssignment, user_id: int
    ) -> AssignmentOwnerView:
        with Session(StorageService.engine) as session:
            level_id = (
                session.query(Level).filter_by(name=new_assignment.level).one().id
            )
            location_id = (
                session.query(Location).filter_by(name=new_assignment.location).one().id
            )
            ...
            try:
                ...
                assignment.subjects = (
                    ...
                    .filter(Subject.name.in_(new_assignment.subjects))
                    .all()
                )
                ...
                assignment.available_slots = (
                    session.query(AssignmentSlot)
                    .filter(AssignmentSlot.assignment_id == assignment.id)
                    .all()
                )

                session.commit()
                session.refresh(assignment)
                return AssignmentLogic.convert_assignment_to_view(
                    session, assignment, ViewType.OWNER
                )
                ...
                if isinstance(e.orig, ForeignKeyViolation):
                    ...
                    raise HTTPException(status_code=500, detail=e.orig.args[0])

    @staticmethod
    def get_assignment_by_id(
        id: int, user_id: int = None
    ) -> AssignmentOwnerView | AssignmentPublicView:
        with Session(StorageService.engine) as session:
            assignment = StorageService.find(
                session, {"id": id}, Assignment, find_one=True
            )
            if not assignment:
                raise HTTPException(status_code=404, detail="Assignment not found")
            view_type = (
                ViewType.OWNER if assignment.owner_id == user_id else ViewType.PUBLIC
            )
            return AssignmentLogic.convert_assignment_to_view(
                session, assignment, view_type, user_id
            )

    @staticmethod
    def update_assignment_by_id(
        id: str | int,
        assignment_update: NewAssignment,
        assert_user_authorized: Callable[[int], None],
    ) -> AssignmentOwnerView:
        with Session(StorageService.engine) as session:
            ...
            level_id = (
                session.query(Level).filter_by(name=assignment_update.level).one().id
            )
            ...
            assignment = StorageService.update(
                session, {"id": id}, assignment_dict, Assignment
            )
            if not assignment:
                raise HTTPException(status_code=404, detail="Assignment not found")
            ...
            assignment.subjects = (
                session.query(Subject)
                .filter(Subject.name.in_(assignment_update.subjects))
                .all()
            )

            # Clear slots
            StorageService.delete(
                session, {"assignment_id": assignment.id}, AssignmentSlot
            )
            # Update available slots
            for slot in assignment_update.available_slots:
                assignment_slot = StorageService.insert(
                    session,
                    AssignmentSlot(
                        day=slot.day,
                        start_time=slot.start_time,
    ...
    def request_assignment(
        new_assignment_request: NewAssignmentRequest, tutor_id: str | int, origin: str
    ) -> None:
        with Session(StorageService.engine) as session:
            # Find the assignment
            statement = session.query(Assignment).filter_by(
                id=new_assignment_request.assignment_id
            )
            ...
            if (
                ...
                or assignment.tutor_id is not None
            ):
                raise HTTPException(
                    status_code=400, detail="Assignment is not open for requests"
                )
            if assignment.owner_id == tutor_id:
                raise HTTPException(
                    status_code=400, detail="Tutor cannot request their own assignment"
                )
            ...
            try:
                assignment_request = AssignmentRequest(
                    ...
                    or assignment.estimated_rate_hourly,
                    requested_duration=new_assignment_request.requested_duration
                    or assignment.lesson_duration,
                )
                ...
                if isinstance(e.orig, ForeignKeyViolation):
                    ...
                    raise HTTPException(
                        status_code=409, detail="Assignment request already exists"
                    )
                else:
                    raise e

    @staticmethod
    def change_assignment_request_status(
        assignment_request_id: str | int,
        status: str,
        assert_user_authorized: Callable[[int], None],
    ) -> None:
        with Session(StorageService.engine) as session:
            ...
            assignment_request = (
                session.query(AssignmentRequest)
                .options(joinedload(AssignmentRequest.assignment))
                .filter_by(id=assignment_request_id)
                .first()
            )
            if not assignment_request:
                raise HTTPException(
                    status_code=404, detail="Assignment request not found"
                )
            ...
            if assignment.tutor_id is not None:
                raise HTTPException(
                    status_code=400, detail="Assignment already has a tutor assigned"
                )
    ...
    def get_created_assignments(user_id: int) -> list[AssignmentOwnerView]:
        ...
        with Session(StorageService.engine) as session:
            assignments = StorageService.find(
                session, {"owner_id": user_id}, Assignment, find_one=False
            )
            if not assignments:
                return []
    ...
    def get_applied_assignments(user_id: int) -> list[AssignmentPublicView]:
        ...
        with Session(StorageService.engine) as session:
            assignments = (
                ...
                .join(Assignment.assignment_requests)
                .filter(AssignmentRequest.tutor_id == user_id)
                .all()
            )
            if not assignments:
                return []
    ...
    def accept_assignment_request(assignment_request_id: int) -> tuple[int, int]:
        with Session(StorageService.engine) as session:
            ...
            if not assignment_request:
                ...
                raise HTTPException(
                    status_code=409,
                    detail="Assignment request is not pending. Cannot accept.",
                )
    ...
    def get_assignment_owner_id(assignment_request_id: int) -> User:
        with Session(StorageService.engine) as session:
            ...
            if not assignment:
                raise HTTPException(
                    status_code=404, detail="Assignment not found for the given request"
                )
    ...
    def get_lesson_duration(assignment_request_id: int) -> int:
        with Session(StorageService.engine) as session:
            assignment_request = (
                session.query(AssignmentRequest)
                .options(joinedload(AssignmentRequest.assignment))
                .filter(AssignmentRequest.id == assignment_request_id)
                .first()
            )
            if not assignment_request:
                raise HTTPException(
    ...
    @staticmethod
    def get_assignment_request_by_id(
        assignment_request_id: int, assert_user_authorized: Callable[[int], None]
    ) -> AssignmentRequestView:
        with Session(StorageService.engine) as session:
            assignment_request = (
                session.query(AssignmentRequest)
    ...
    @staticmethod
    async def update_assignment_request_by_id(
        assignment_request_id: int,
        req: ModifiedAssignmentRequest,
        assert_user_authorized: Callable[[int], None],
    ) -> AssignmentRequestView:
        with Session(StorageService.engine) as session:
            assignment_request = (
            ...
            assignment_request.requested_rate_hourly = (
                req.requested_rate_hourly or assignment_request.requested_rate_hourly
            )
            assignment_request.requested_duration = (
                req.requested_duration or assignment_request.requested_duration
            )
            ...
            chat_message = await AssignmentLogic.create_and_store_message(
                session, assignment_request
            )
            ...
            asyncio.create_task(
                ChatLogic.send_private_message(chat_message=chat_message)
            )

            return AssignmentLogic.get_assignment_request_by_id(
                assignment_request.id, assert_user_authorized
            )

    @staticmethod
    async def create_and_store_message(
        session: Session, assignment_request: AssignmentRequest
    ) -> ChatMessage:
        # Send a chat message to the assignment owner
        assignment = assignment_request.assignment
        chat = ChatLogic.get_or_create_private_chat(
            assignment.owner_id, assignment_request.tutor_id
        )
        chat_id = chat.id
        ...
        new_chat_message = NewChatMessage(
            chat_id=chat_id, content=message_content, message_type="tutor_request"
        )

        # Store the chat message and get the message ID
        chat_message = await ChatLogic.store_private_message(
            session, new_chat_message, assignment_request.tutor_id
        )
