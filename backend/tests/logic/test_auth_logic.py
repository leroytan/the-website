...
from api.storage.models import EmailVerificationStatus
...
class TestLogic:
    ...
    def test_handle_login_success(
        self, mock_auth_service, mock_storage_service, sample_user_data
    ):
    ...
    @patch("api.logic.auth_logic.StorageService")
    @patch("api.logic.auth_logic.AuthService")
    def test_handle_login_user_not_found(
        self, mock_auth_service, mock_storage_service, sample_user_data
    ):
    ...
    @patch("api.logic.auth_logic.StorageService")
    @patch("api.logic.auth_logic.AuthService")
    def test_handle_login_wrong_password(
        self, mock_auth_service, mock_storage_service, sample_user_data
    ):
    ...
    @patch("api.logic.auth_logic.StorageService")
    @patch("api.logic.auth_logic.AuthService")
    def test_handle_login_pending_verification(
        self, mock_auth_service, mock_storage_service, sample_user_data
    ):
    ...
    @patch("api.logic.auth_logic.StorageService")
    @patch("api.logic.auth_logic.AuthService")
    def test_handle_login_waitlisted(
        self, mock_auth_service, mock_storage_service, sample_user_data
    ):
    ...
    def test_handle_signup_success_pending_verification(
        self,
        mock_email_service,
        mock_auth_service,
        ...
        with patch("api.logic.auth_logic.Session") as mock_session_class:
            ...
            mock_auth_service.create_email_confirmation_token.return_value = (
                "confirmation_token"
            )

            # Mock email service methods
            mock_email_service.create_email_confirmation_link.return_value = (
                "confirmation_link"
            )
    ...
    @patch("api.logic.auth_logic.StorageService")
    @patch("api.logic.auth_logic.AuthService")
    def test_handle_signup_user_already_exists(
        self, mock_auth_service, mock_storage_service, sample_user_data
    ):
    ...
    @patch("api.logic.auth_logic.StorageService")
    @patch("api.logic.auth_logic.AuthService")
    def test_handle_signup_integrity_error(
        self, mock_auth_service, mock_storage_service, sample_user_data
    ):
    ...
    @patch("api.logic.auth_logic.StorageService")
    @patch("api.logic.auth_logic.AuthService")
    def test_get_current_user_token_version_mismatch(
        self, mock_auth_service, mock_storage_service, sample_user_data
    ):
    ...
    def test_refresh_tokens_success(
        ...
        with patch("api.logic.auth_logic.Session") as mock_session_class:
            ...
            assert result == mock_token_pair
            mock_auth_service.verify_token.assert_called_once_with(
    ...
    def test_forgot_password_success(
        self, mock_email_service, mock_auth_service, mock_storage_service
    ):
        ...
        with patch("api.logic.auth_logic.Session") as mock_session_class:
            ...
            # Test the method
            result = AuthLogic.forgot_password(
                "http://localhost:3000", forgot_password_request
            )
    ...
    def test_reset_password_success(self, mock_auth_service, mock_storage_service):
        ...
        mock_user.email = "test@example.com"
        mock_user.token_version = 0
        mock_user.password_reset_token = (
            "valid_reset_token_that_is_long_enough_for_validation_32_chars"
        )
        mock_user.password_reset_token_expires_at = datetime.now(
            timezone.utc
        ) + timedelta(minutes=5)
    ...
    def test_reset_password_invalid_token(
        self, mock_auth_service, mock_storage_service
    ):
        """Test password reset with invalid token"""
        # Mock token verification to raise exception
        mock_auth_service.verify_password_reset_token.side_effect = ValueError(
            "Invalid token"
        )
    ...
    async def test_handle_google_login_signup_success(self, mock_auth_service):
        ...
        mock_token_pair = Mock()
        mock_auth_service.authenticate_google_user = AsyncMock(
