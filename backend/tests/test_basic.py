...
class TestBasicSetup:
    ...
    def test_test_structure(self):
        ...
        for test_dir in test_dirs:
            assert os.path.exists(f"tests/{test_dir}"), f"Test directory tests/{test_dir} does not exist"
            assert os.path.exists(f"tests/{test_dir}/__init__.py"), f"__init__.py missing in tests/{test_dir}"
        ...
        assert not os.path.exists("tests/integration"), "Integration directory should not exist"
        ...
        for test_file in test_files:
            if test_file.endswith('.py') and 'test_' in test_file:
                ...
                assert filename.startswith('test_'), f"Test file {filename} should start with 'test_'"
        ...
        assert len(test_files) >= 15, f"Expected at least 15 test files, found {len(test_files)}"
