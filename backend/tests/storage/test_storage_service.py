...
class TestStorageService:
            ...
            with patch("api.storage.storage_service.inspect") as mock_inspect:
                with patch("api.storage.storage_service.Base") as mock_base:
                    with patch("api.storage.storage_service.Session") as mock_session:
                        with patch(
                            "api.storage.storage_service.seed_database"
                        ) as mock_seed:
                            with patch(
                                "api.storage.storage_service.settings"
                            ) as mock_settings:
                                with patch(
                                    "api.storage.storage_service.insert_test_data"
                                ) as mock_insert:
        ...
        with patch("api.storage.storage_service.database_exists") as mock_exists:
            with patch("api.storage.storage_service.create_database") as mock_create:
                with patch("api.storage.storage_service.inspect") as mock_inspect:
                    with patch("api.storage.storage_service.Base") as mock_base:
                        with patch(
                            "api.storage.storage_service.Session"
                        ) as mock_session:
                            with patch(
                                "api.storage.storage_service.seed_database"
                            ) as mock_seed:
    ...
    def test_find_with_dict_query(self):
        ...
        with patch("api.storage.storage_service.Session") as mock_session_class:
            with patch("api.storage.storage_service.select") as mock_select:
                with patch(
                    "api.storage.storage_service.StorageService.engine"
                ) as mock_engine:
                    ...
                    result = StorageService.find(
                        mock_session, {"email": "test@example.com"}, User
                    )

                    assert result == [mock_user]
                    mock_statement.filter_by.assert_called_once_with(
            ...
            with patch("api.storage.storage_service.select") as mock_select:
                with patch("api.storage.storage_service.and_") as mock_and:
                    with patch(
                        ...
                        assert result == [mock_user]
                        mock_statement.where.assert_called_once_with(
        ...
        with patch("api.storage.storage_service.Session") as mock_session_class:
            with patch("api.storage.storage_service.select") as mock_select:
                with patch(
                    ...
                    result = StorageService.find(
                        mock_session, {"email": "test@example.com"}, User, find_one=True
                    )
    ...
    def test_find_invalid_query_type(self):
        ...
        with pytest.raises(
            ValueError,
            match="Query must be a dictionary or a list of ColumnElement objects",
        ):
            ...
            with patch("api.storage.storage_service.select") as mock_select:
                with patch(
                    ...
                    with patch("api.storage.storage_service.Utils") as mock_utils:
                        ...
                        mock_utils.validate_non_empty.side_effect = ValueError(
                            "Empty query"
                        )
                ...
                with patch("api.storage.storage_service.and_") as mock_and:
                    with patch("api.storage.storage_service.or_") as mock_or:
                        with patch(
                            ...
                            result = StorageService.find_any(
                                mock_session, queries, User
                            )
                ...
                with patch("api.storage.storage_service.and_") as mock_and:
                    with patch("api.storage.storage_service.or_") as mock_or:
                        with patch(
                            ...
                            with patch(
                                "api.storage.storage_service.Utils"
                            ) as mock_utils:
                                # Mock session and empty result
                                mock_session_instance = Mock()
    ...
    def test_update(self):
        ...
        with patch("api.storage.storage_service.update") as mock_update_func:
            with patch("api.storage.storage_service.and_") as mock_and:
                with patch(
                    "api.storage.storage_service.StorageService.find"
                ) as mock_find:
                    ...
                    result = StorageService.update(
                        mock_session, {"id": 1}, {"email": "updated@example.com"}, User
                    )
