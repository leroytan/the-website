...
from api.storage.seed import seed_subjects, seed_levels, seed_locations, seed_database
...
class TestSeed:
    ...
    def test_seed_locations_contains_expected_areas(self):
        ...
        expected_locations = [
            "Ang Mo Kio", "Bedok North", "Bedok South", "Bishan",
            "Bukit Batok", "Bukit Merah", "Clementi", "Jurong East",
            "Orchard", "Tampines", "Woodlands"
        ]
        ...
        assert len(found_locations) >= 5, f"Expected at least 5 common locations, found: {found_locations}"
    ...
    def test_seed_database_success(self, mock_seed_subjects, mock_seed_levels, mock_seed_locations):
    ...
    @patch('api.storage.seed.seed_levels')
    @patch('api.storage.seed.seed_subjects')
    def test_seed_database_exception_handling(self, mock_seed_subjects, mock_seed_levels, mock_seed_locations):
